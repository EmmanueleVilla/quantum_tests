    ...
    while True:
        ...
        for i in range(len(individual)):
            ...
            if new_fitness > fitness:
                ...
                individual[i] = old
        if found:
            offset = base_offset(fitness)  # base_offset(fitness)
        else:
            # randomly increase or decrease the offset
            offset += -0.01 if random.random() < 0.5 else 0.01
            if offset < 0.0001:
...
def main():
    # Create the data
    train_features, train_labels, test_features, test_labels = create_dataset(100, negative_value=0, m=2, n=4)
